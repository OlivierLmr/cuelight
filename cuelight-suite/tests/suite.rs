...
/// A run that ends at the time limit is normal: an algorithm driven by timers, a failure detector
/// above all, keeps going until something stops it. It must still be judged.
#[test]
fn a_run_that_reaches_the_limit_is_still_judged() {
    if !have_python() {
        return;
    }
    let (_, text) = demo_run("busy", "busy.py", &[]);
    assert!(text.contains("quiet: 0/4 seeds passed"), "{text}");
    // Judged on its property, not dismissed for having reached the limit.
    assert!(text.contains("every-poke-answered"), "{text}");
}
